          ...
          <div className="mt-4 space-y-2">
            {myListItems.map((item, index) => (
              <div key={index}>
                <Button
                  variant="ghost"
                  className="w-full min-h-[40px] flex items-center hover:bg-orange-500/20 py-3 px-4 whitespace-normal text-left relative" // Added relative for overlay, items-center for vertical centering
                  onClick={() => toggleDropdown(index)}
                >
                  <ChevronDown
                    size={16}
                    className={`text-white mr-3 flex-shrink-0 transition-transform ${openDropdown === index ? 'rotate-180' : ''}`}
                  />
                  <div className="flex-1"> {/* Added flex-1 to allow text to take available space */}
                    <span className="text-sm font-semibold text-black hover:text-white whitespace-normal leading-5"> {/* Added leading-5 */}
                      {typeof item === 'string' ? item : item.name}
                    </span>
                  </div>
                  <div className="absolute inset-0 bg-black/0 hover:bg-black/10 transition-colors duration-300"></div> {/* Overlay */}
                </Button>
                  ...
                  <div className="ml-6 mt-2 space-y-4">
                    {item.subItems.map((subItem, subIndex) => (
                      <div key={subIndex}>
                        <Button
                          variant="ghost"
                          className="w-full min-h-[40px] flex items-center hover:bg-orange-500/20 py-2 px-4 whitespace-normal text-left relative" // Added relative, items-center
                        >
